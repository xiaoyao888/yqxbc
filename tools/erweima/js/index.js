...
function tabfn(e, t, n) {
    n = $.extend({name1: "active", name2: "show"}, n);
    e.each(function (r) {
        $(this).click(function () {
            e.removeClass(n.name1);
            $(this).addClass(n.name1);
            t.removeClass(n.name2).eq(r).addClass(n.name2);
            if ($(this).index() == 7 && !("baiduMap" in window)) {
                $.getScript("/js/2013/bdmap.js", function () {
                    baiduMap("bdmap", canvasObj || null)
                })
            }
        })
    })
...
function elemfocus(e, t) {
    t = $.extend({hover: "hover", active: "active"}, t);
    e.bind({
        mouseenter: function () {
            $this = $(this);
